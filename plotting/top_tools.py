...
def get_args():
    parser = argparse.ArgumentParser(description = 'Show the top\
            tools being used')
...
def main():
    ...
    for line in open(args.datafile, 'r'):
        ...
        plot_data[tool][network] = count
        absolute_total += count
    for tool in plot_data:
        total = 0
        for network in plot_data[tool]:
            total += plot_data[tool][network]
        plot_data[tool]['total'] = total
        tool_usage.append((tool,total))
    
    sorted_tool_usage = sorted(tool_usage, key = lambda data :\
            data[1], reverse = True)
    for item in sorted_tool_usage[:20]:
        print item[1]
