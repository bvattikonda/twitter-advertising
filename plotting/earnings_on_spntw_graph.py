...
def get_args():
    ...
    parser.add_argument('--userinfo', required = True,\
        help = 'Text file of sponsored tweets users information')
...
def get_earnings(usersfilename):
    ...
    for line in usersfile:
        ...
        if 'available' in x and x['available']:
            earnings.append(x['price'])
    return earnings
...
def main():
    ...
    subplot.set_title('Earnings by users on Sponsored Tweets', fontproperties = TITLE_PROP) 
    subplot.set_ylim(0, 1)
    subplot.set_xlim(0, 200)
    print_figure(fig, args.destination_dir, 'earnings_on_spntw', args.extension)
